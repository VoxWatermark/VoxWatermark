...
def pert_background_noise(waveform, snr_db, noise_file_path):
    ...
    # Convert the noise to mono (if stereo)
    if noise.size(0) > 1:
...
def apply_no_box_pert(output_dir, common_perturbation):
    if common_perturbation == "time_stretch":
        ...
        for speed_factor in speed_factor_list:
            ...
            batch_SNRs = []
            visqol = api_visqol()
            output_dir_pert = f'/content/perturbed_no_box_fol/time_stretch_speed_{speed_factor}'
            os.makedirs(output_dir_pert, exist_ok=True)

            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(file_list, desc=f"Applying time stretch (speed {speed_factor})")
            ...
            for file in progress_bar:
                try:
                    ...
                    output_path = os.path.join(output_dir_pert, file)
            ...
            print(
                f"Mean ViSQOL={np.mean(batch_visqol_scores):.3f} | "
                f"Mean SNR={np.mean(batch_SNRs):.2f} dB"
            )
        ...
        for snr in snr_values:
            ...
            output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_{snr}'
            file_list = [file for file in os.listdir(output_dir) if file.endswith('.wav')]
            progress_bar = tqdm(file_list, desc=f"Applying {common_perturbation} (snr {snr})")
                    ...
                    for noise_file in noise_files:
                        ...
                        for file in progress_bar:
                          try:
                            ...
                            output_path = os.path.join(output_dir_pert_noise, file)
                            if waveform_pert.ndim == 1:
                                waveform_pert = waveform_pert.unsqueeze(0)
                            elif waveform_pert.shape[0] != 1:
                                waveform_pert = waveform_pert[0:1]
        ...
        for bitrate in bitrate_list:
            ...
            output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_bitrate_{bitrate*16}k'
            os.makedirs(output_dir_pert, exist_ok=True)
        ...
        for bandwidth in bandwidth_values:
            ...
            output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_bandwidth_{bandwidth}'
            os.makedirs(output_dir_pert, exist_ok=True)
      ...
      for quantization_bit in quantization_levels:
          ...
          output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_quantization_bit_{quantization_bit}'
          os.makedirs(output_dir_pert, exist_ok=True)
        ...
        for ratio in ratio_list:
            ...
            output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_ratio_{ratio}'
            os.makedirs(output_dir_pert, exist_ok=True)
        ...
        for window_size in window_list:
            ...
            output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_window_size_{window_size}'
            os.makedirs(output_dir_pert, exist_ok=True)
        ...
        for decay in decay_list:
            ...
            output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_decay{decay}'
            os.makedirs(output_dir_pert, exist_ok=True)
        ...
        for bitrate in bitrate_list:
            ...
            output_dir_pert = f'/content/perturbed_no_box_fol/{common_perturbation}_bitrate_{bitrate}'
            os.makedirs(output_dir_pert, exist_ok=True)
...
dataset_dir = "/content/clean"

perturbations= ["time_stretch", "gaussian_noise", "background_noise", "opus", "encodec", "quantization", 
                "highpass", "lowpass", "smooth", "echo", "mp3", "aac", "dynamic_compression", "dynamic_expansion",
